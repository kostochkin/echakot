...
data LoggerF m a b c =
  LoggerF (LogMessage (a -> b)) (Logger m b c)

data LoggerV m a b =
  LoggerV (LogMessage a) (Logger m a b)

data LoggedA m a b c =
  LoggedA (m a) (LoggerF m a b c)
...
joinLoggers xs =
  Logger
    ...
    , currentLogLevel' = maximum $ map currentLogLevel' xs
    }
  where
...
logBefore mc l lm = maybe mc k' (bounce l lm)
  where
    k' _ = do
      _ <- runLogger l lm
      mc
...
logMITM ::
  ...
  -> LogMessage (a -> b)
  -> m c
logMITM before after l lm = maybe (before >>= after) k' (bounce l lm)
  where
    k' _ = do
      x <- before
