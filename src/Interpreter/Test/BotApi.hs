module Interpreter.Test.BotApi
  ( interpret
  ) where
...
import Language.Bot
  ( BotApi
  , BotApiF(BotLog, EchoMessage, GetCurrentRepeats, GetMessages,
        RunTimes, SelectAction, SetRepeats, ShowHelp, ShowKeyboard,
        TellCurrentRepeats)
  )
...
interpret ::
     (Read i, Show i, Eq i, Ord i, Num i)
  => BotApi String i ()
  -> S i
  -> [i]
  -> [String]
...
interpret' (Free bf) s0 k0 acc0 = free bf s0 k0 acc0
  where
    ...
    free (RunTimes i p f) s k acc = interpret'' i s acc
      where
        interpret'' i' s1 acc1
          | i' < 1 = interpret' f s1 k acc1
          | otherwise =
            let (s1', acc1') = interpret' p s k acc1
             in interpret'' (i' - 1) s1' acc1'
