...
main = do
    ...
    testRight readLoggerTest configApp "Test read logger form config"
    testLeft  readLoggerTest configAppLoggerFail "Test read logger from config must fail"
...
testRight f a s = putStrLn $ case f a of
    Right _ -> s ++ ": OK"
    Left _ ->  s ++ ": FAIL!"

testLeft :: (a -> Either b c) -> a -> String -> IO ()
testLeft f a s = putStrLn $ case f a of
    Right _ -> s ++ ": FAIL!"
    Left _ ->  s ++ ": OK"
...
loggerFail = [ [("type", "stdio"),("logLevel", "None")]
             , [("type", ""),("logLevel", "Debug"),("filename", "echobot-test-fail.log")]]

messengerC = [ ("type", "stdio")
             , ("showingKeyboardMessage", "keyboard")]

botGeneral = [ ("repeats", "1")
             , ("repeatsMessage", "repeats")
             , ("helpMessage", "help")]

configApp = [("loggers", show loggers), ("messenger", show messengerC), ("botGeneral", show botGeneral)]

configAppLoggerFail = [("loggers", show loggerFail), ("messenger", show messengerC), ("botGeneral", show botGeneral)]


readLoggerTest :: [(String, String)] -> Either String (IO (LL.Logger IO String ()))
